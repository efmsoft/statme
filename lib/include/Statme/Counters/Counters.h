...
#define INIT_CNTR_HOLDER(holder, self, category) \
  holder(Counters::Manager::GetInstance()->AddCounter(std::format("{}", (const void*)self), category))

#define THREAD_COUNTER(name) \
  Counters::Holder holder(Counters::Manager::GetInstance()->AddCounter(name, CNTR_CATEGORY_THREAD)); \
